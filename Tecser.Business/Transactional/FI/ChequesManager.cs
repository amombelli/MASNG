...
namespace Tecser.Business.Transactional.FI
{
    ...
    {
        public int AddNewChequeConstructor(string tipoLx, string numeroReciboIngreso, DateTime fechaRecibido,
            int idCliente, DateTime fechaAcreditacion, decimal importe, string numero, string numeroBanco,
            bool interior,
            string numeroCuit, string titular = null, string moneda = "ARS", string observacion = null,
            int idCobranza = 0, int idClienteRecibido = 0, string codigoPostal = null)
        {
        ...
        private int AddCheque(T0154_CHEQUES ch)
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                if (ch.IDCHEQUE == 0)
        ...
        public bool LiberaCheque(int idCheque)
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                var data = db.T0154_CHEQUES.SingleOrDefault(c => c.IDCHEQUE == idCheque);
        ...
        public List<TsCheques1> GetListaChequesFiltrada(string lx, bool verDisponible = true,
            bool verNoDisponible = false,
            string numeroCheque = "", int idCheque = -1, DateTime? menorIgualaFecha = null, bool verInterior = true,
            bool verNoInterior = true)
        {
