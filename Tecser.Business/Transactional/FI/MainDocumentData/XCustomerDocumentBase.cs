...
namespace Tecser.Business.Transactional.FI.MainDocumentData
{
    ...
    public class XCustomerDocumentBase
    {
            ...
            string comentario = null)
        {
            ...
            {
                CLINUM = header400.Cliente,
                CLITXT = header400.RAZONSOC,
            ...
            {
                ncdh.TOTAL_ARS_NETO = header400.TotalFacturaN;
                ncdh.TOTAL_USD_NETO = Decimal.Round(((decimal) header400.TotalFacturaN/(decimal) header400.TC), 2);
            }
            else
            {
                ncdh.TOTAL_USD_NETO = header400.TotalFacturaN;
                ncdh.TOTAL_ARS_NETO = Decimal.Round(((decimal)header400.TotalFacturaN * (decimal)header400.TC), 2);
            }
        ...
        private void UpdateImportes()
        {
            ...
            foreach (var item in I)
            {
                if (H.FacturaMoneda == "ARS")
                {
                    ...
                    importeBruto += item.PRECIOT_FACT_ARS.Value;

                    if (item.IVA21.Value)
                    {
                        importeImponible += item.PRECIOT_FACT_ARS.Value;
                    }
                }
                else
                {
                    ...
                    importeBruto += item.PRECIOT_FACT_USD.Value;

                    if (item.IVA21.Value)
                    {
                        importeImponible += item.PRECIOT_FACT_USD.Value;
                    }
                }
            }
            ...
            if (H.FacturaMoneda == "ARS")
            {
                _nh.TOTAL_ARS_NETO = H.TotalFacturaN;
                _nh.TOTAL_USD_NETO = Decimal.Round(((decimal) H.TotalFacturaN/(decimal) H.TC), 2);
            }
            else
            {
                _nh.TOTAL_USD_NETO = H.TotalFacturaN;
                _nh.TOTAL_ARS_NETO = Decimal.Round(((decimal) H.TotalFacturaN*(decimal) H.TC), 2);
            }
        }
        ...
        public FacturaId GrabaDocumentoToDatabase()
        {
            ...
            Boolean existeF400 = false;
            Boolean existeN300 = false;
            ...
            if (H.IDFACTURA > 0)
            {
                using (var db = new TecserData(GlobalApp.CnnApp))
                {
                    ...
                    if (existeN300)
                    {
                        ...
                        ncdH.TOTAL_ARS_NETO = _nh.TOTAL_ARS_NETO;
                        ncdH.TOTAL_USD_NETO = _nh.TOTAL_USD_NETO;
...
#pragma warning restore CS0219 // The variable 'x' is assigned but its value is never used
        ...
        //public void UpdateAsientoData(int idCtaCte, int asientoNAS, int asientox1x2,string numeroDocumentoCompleto)
        //{
        //    H.NAS = asientoNAS;
        //    H.NASX1X2 = asientox1x2;
        //    H.IdCtaCte = idCtaCte;
        //    using (var db = new TecserData(GlobalApp.CnnApp))
        //    {
        ...
        //        {
        //            i.NUMFACTURA = numeroDocumentoCompleto;
        //            i.NAS
        //        }
        //    }
        ...
        //completa datos AFIP, completa totales, etc
        //base imponible, descuento importe, ret iibb,
    }
}