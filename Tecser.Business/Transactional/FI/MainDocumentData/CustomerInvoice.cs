    ...
    public class CustomerInvoice : CustomerMainDocument
    {
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                foreach (var it in dataR)
                {
                    ...
                    var tmp = new T0401_FACTURA_I
                    {
                        ...
                        ITEM = it.MATERIALAKA,
                        IVA21 = it.IVA21,
                        KGDESPACHADOS_R = it.KGDESPACHADOS_R.Value,
                    ...
                    if (H.TIPOFACT == "L1")
                    {
                        tmp.PRECIOU_COTIZ = dataSO.PRECIO1.Value;
                        tmp.PRECIOT_COTIZ = dataSO.PRECIO1.Value*tmp.KGDESPACHADOS_R.Value;
                    }
                    else
                    {
                        tmp.PRECIOU_COTIZ = dataSO.PRECIO2.Value;
                        tmp.PRECIOT_COTIZ = dataSO.PRECIO2.Value * tmp.KGDESPACHADOS_R.Value;
                    }
        ...
        private void UpdateTotalsHeader_InMemory()
        {
            ...
            if (H.FacturaMoneda == "ARS")
            {
                foreach (var it in I)
                {
                    importeBruto += it.PRECIOT_FACT_ARS.Value;
                    if (it.IVA21 == true)
                    {
                        importeBaseImponible += it.PRECIOT_FACT_ARS.Value;
                        importeIva21 += (it.PRECIOT_FACT_ARS.Value*(decimal) 0.21);
                    }
                }
            }
            else
            {
                ...
                foreach (var it in I)
                {
                    importeBruto += it.PRECIOT_FACT_USD.Value;
                    if (it.IVA21 == true)
                    {
                        importeBaseImponible += it.PRECIOT_FACT_USD.Value;
                        importeIva21 += (it.PRECIOT_FACT_USD.Value*(decimal) 0.21);
                    }
                }
            }
...
#pragma warning restore CS0219 // The variable 'importeDescuento' is assigned but its value is never used
            ...
            foreach (var it in I)
            {
                ...
                if (it.MONEDA_COTIZ == "ARS")
                {
                    ...
                    it.PRECIOU_FACT_USD = decimal.Round((decimal)it.PRECIOU_FACT_ARS/H.TC.Value, 3);
                    it.PRECIOT_FACT_USD = decimal.Round((decimal) it.PRECIOT_FACT_ARS/ H.TC.Value, 2);
                    
                }
                else
                {
                    //moneda de cotizacion USD recalcula los PreciosU/Total en ARS
                    it.PRECIOU_FACT_ARS = decimal.Round((decimal) it.PRECIOU_FACT_USD* H.TC.Value, 3);
                    it.PRECIOT_FACT_ARS = decimal.Round((decimal)it.PRECIOT_FACT_USD*H.TC.Value,2);
                }
                ...
                if (H.FacturaMoneda == "ARS")
                {
                    ...
                    if (it.IVA21 == true)
                    {
                        ...
                        importeBaseImponible += it.PRECIOT_FACT_ARS.Value *(1-H.DescuentoPorc.Value);
                    }
                    else
                    {
                        importeNoImponible += it.PRECIOT_FACT_ARS.Value*(1 - H.DescuentoPorc.Value);
                    }
                }
                else
                {
                    ...
                    if (it.IVA21 == true)
                    {
                        importeBaseImponible += it.PRECIOT_FACT_USD.Value * (1 - H.DescuentoPorc.Value);
                    }
                    else
                    {
                        importeNoImponible += it.PRECIOT_FACT_USD.Value * (1 - H.DescuentoPorc.Value);
                    }
                }
            }
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                foreach (var i in fi)
                {
                    ...
                    if (i.MONEDA_COTIZ == "ARS")
                    {
                        ...
                        i.PRECIOT_FACT_ARS = i.PRECIOU_COTIZ*i.KGDESPACHADOS_R;
                        i.PRECIOU_FACT_USD = i.PRECIOU_COTIZ*H.TC;
                        i.PRECIOT_FACT_USD = i.PRECIOU_FACT_USD*i.KGDESPACHADOS_R;
                    }
                    else
                    {
                        i.PRECIOU_FACT_ARS = i.PRECIOU_COTIZ*H.TC;
                        i.PRECIOT_FACT_ARS = i.PRECIOU_COTIZ*i.KGDESPACHADOS_R*H.TC;
                        i.PRECIOU_FACT_USD = i.PRECIOU_COTIZ;
                        i.PRECIOT_FACT_USD = i.PRECIOU_COTIZ*i.KGDESPACHADOS_R;
                    }
                    I.Add(i);
                }
                db.SaveChanges();
