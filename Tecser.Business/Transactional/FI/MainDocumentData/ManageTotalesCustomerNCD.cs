    ...
    public class ManageTotalesCustomerNcd : CustDocTotalesStruct
    {
        ...
        public void CalculaAfterUnitPriceIsChanged(int idItem,decimal newUnitPrice)
        {
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                foreach (var it in items)
                {
                    ...
                    {
                        brutoOri += it.PRECIOT_FACT_ARS.Value;
                        ...
                        {
                            brutoNew += it.PRECIOT_FACT_ARS.Value;
                            if (it.IVA21.Value)
                            {
                                baseImponibleNew += it.PRECIOT_FACT_ARS.Value;
                            }
                        }
                    }
                    else
        ...
        private void CalculoBaseImponible()
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                {
                    sum =
                        db.T0401_FACTURA_I.Where(c => c.IDFactura == IdFactura && c.IVA21.Value == true)
