    ...
    { 
        ...
        public Retorno GetData()
        {
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                var x207 =db.T0207_SPLITFACTURAS.Where(c => c.TIPO == "L1" && c.MONTO_APLICADO == 0 && c.CLIENTE == _idCli).ToList();
                ret.ImporteL1_207=!x207.Any() ? 0 : x207.Sum(c => c.MONTO.Value);

                x207 =db.T0207_SPLITFACTURAS.Where(c => c.TIPO == "L2" && c.MONTO_APLICADO == 0 && c.CLIENTE == _idCli).ToList();
                ret.ImporteL2_207 = !x207.Any() ? 0 : x207.Sum(c => c.MONTO.Value);
