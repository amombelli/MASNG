...
namespace Tecser.Business.Transactional.FI.Cobranza
{
    ...
    //-----------------------------------------------------------
        ...
        public List<T0205_COBRANZA_H> GetListCobranzasByCustomerId()
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                return db.T0205_COBRANZA_H.Where(c => c.CLIENTE == this.IdCliente).OrderByDescending(c => c.IDCOB).ToList();
            }
        }
        ...
        public void DesimputaCobranza()
        {
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                if (montoImputado > 0)
                {
                    ...
                    {
                        DesimputaFacturaCobranza(it.IDCTACTE,it.MONTO_APLICADO.Value);
                        importeDesimputado += it.MONTO_APLICADO.Value;
                        it.MONTO_APLICADO = 0;
                        it.NRECIBO = null;
                        it.PFECHA = null;
                        db.SaveChanges();
                    ...
                    desimputaCobranza.SALDOFACTURA = cobi.Monto.Value*-1;
