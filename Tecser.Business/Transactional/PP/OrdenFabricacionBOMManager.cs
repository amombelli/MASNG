...
namespace Tecser.Business.Transactional.PP
{
    ...
    public class OrdenFabricacionBOMManager
    {
        ...
        public void UpdateT0073(int numeroOF, List<T0073_FORMULA_PRINT> data)
        {
        ...
        /// </summary>
        public void CalculaBatchSize(int idOF, decimal totalFabricar, bool asignacionAutomaticaLote = true)
        {
            decimal total = 0;
...
#pragma warning restore CS0219 // The variable 'totalContainers' is assigned but its value is never used
            ...
            using (var db0 = new TecserData(GlobalApp.CnnApp))
            {
                ...
                ////Chequeo de Containers (Empiezan con "B")
                //var containers = listaOFCompleta.Where(c => c.Primario.ToUpper().StartsWith("B")).ToList();
                //{
                //    if (containers.Count > 0)
                //    {
                //        foreach (var cx in containers)
                //        {
                //            totalContainers += cx.CantidadKGReal.Value;
                //        }
                //    }
                //}
                ...
                new ReservaStockOF().LiberaStockReservadoOF(idplan,true,55);
            }
        }
        ...
        public void UpdateKgReal(int idplan, int iditem, decimal newKg,bool SinRecalculo=false)
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                if (result != null)
                {
                    ...
                    //al modificar la cantidad resetea la reserva de lote
                    new StockBatchManagerOF().LiberacionLoteOrdenFabricacionIndividual(idplan, iditem);
                    db.SaveChanges();
                }
            }
        }
