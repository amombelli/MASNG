    ...
    public class ManageGeneracionRemitoFinal
    {
                ...
                foreach (var item in dataRemitoItemList)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            new SalesOrderDeliveryManager().UpdateDeliveryQty(item.IDOV.Value, item.IDOVITEM.Value, item.KGDESPACHADOS.Value);
                            SalesOrderStatusManager.UpdateStatusItem(item.IDOV.Value, item.IDOVITEM.Value);
                            SalesOrderStatusManager.UpdateStatusHeader(item.IDOV.Value);
                        }
        ...
        private bool CompleteItemData(int idRemito, int idItem)
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                new SalesOrderDeliveryManager().FixPriceFromOldModel(item.IDOV.Value, item.IDOVITEM.Value); //todo Remove this fix
                var itemSO =
