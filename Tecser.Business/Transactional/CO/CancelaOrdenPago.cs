                ...
                foreach (var i in op)
                {
                    if (i.CH_CK)
                    {
                        ...
                        new RegisterManager().AddRegisterRecord(i.CUENTA_O, DateTime.Today, "ROP", _idOP.ToString(),
                            tipoPC.Proveedor, i.PROVEEDOR.Value, "OP Reversal", i.MON, i.IMPORTE.Value, 0, i.CH_ID.Value,
                            tipoLx, new CuentasManager().GetGL(i.CUENTA_O), nas, "OPX");
                    }
                    else
                    {
                        new RegisterManager().AddRegisterRecord(i.CUENTA_O, DateTime.Today, "ROP", _idOP.ToString(),
                            tipoPC.Proveedor, i.PROVEEDOR.Value, "OP Reversal", i.MON, i.IMPORTE.Value, 0, 0,
                            tipoLx, new CuentasManager().GetGL(i.CUENTA_O), nas, "OPX");
                    }
                }
            }
