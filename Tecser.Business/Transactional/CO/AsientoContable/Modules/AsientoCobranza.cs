...
{
    public class AsientoCobranza : AsientoCustomerSpecific
    {
        public AsientoCobranza(int idCob, string transactionCode) : base(transactionCode)
        {
            _idCob = idCob;
            LoadHeaderData();
        ...
        protected sealed override void LoadHeaderData()
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                base.IdCliente = _h.CLIENTE.Value;
...
#pragma warning restore CS0219 // The variable 'descripcion2' is assigned but its value is never used
            base.CreacionHeaderAsiento(_h.CUENTA, _h.FECHA.Value, TipoDocumento, _numeroRecibo, descripcion1, _h.MON,
                _h.Monto.Value, _h.TC.Value);

            base.GeneraSegmentoAR(TipoDocumento, _numeroRecibo, _h.CUENTA, DebeHaber.Haber, _h.MON, _h.Monto.Value,
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                foreach (var i in items)
                {
                    ...
                    AddGenericCompleteSegment(TipoDocumento, _numeroRecibo, _h.CUENTA, glItem, descripcion1,
                        descripcion2, _h.MON, DebeHaber.Debe, i.IMP_RECIBO.Value, Tcode, IdCliente, 0,
