...
namespace Tecser.Business.Transactional.CO.AsientoContable
{
    ...
    /// </summary>
    public class AsientoCustomerDocument : AsientoCustomerSpecific
    {
        public AsientoCustomerDocument(int idFactura, string transactionCode)
        ...
        protected sealed override void LoadHeaderData()
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                base.IdCliente = _h400.Cliente.Value;
        ...
        public IdentificacionAsiento AsientoFromCustomerFactura()
        {
            ...
            base.CreacionHeaderAsiento(_h400.TIPOFACT, _h400.FECHA.Value, _h400.TIPO_DOC, NumeroDocumento,
                headerDescription, _h400.FacturaMoneda, _h400.TotalFacturaN, _h400.TC.Value);
        ...
        public IdentificacionAsiento AsientoFromCustomerNotaCredito()
        {
            //Crea Header Asiento NotaDebito
            var headerDescription = "Emision Doc Credito > " + _h400.RAZONSOC;
            NumeroDocumento = new CustomerInvoice(base.Tcode, _idFactura).GetNumeroDocumentoCompleto();
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                foreach (var item in factItems)
                {
                    ...
                    if (_h400.FacturaMoneda == "ARS")
                    {
                        ...
                        importeDh = CostManager.CostBase.GetCostoMercaderiaVendidaInARS(item.ITEM);
                    }
                    else
                    {
                        descripcion1 = "Costo [USD]@" + item.ITEM;
                        importeDh = CostManager.CostBase.GetCostoMercaderiaVendidaInUSD(item.ITEM);
                    }
                    ...
                    AddGenericCompleteSegment(_h400.TIPO_DOC, Header.REFE, _h400.TIPOFACT,
                        ...
                        _h400.Cliente.Value, 0, "T0400_FACTURA_H", _idFactura, null, item.KGDESPACHADOS_R, item.ITEM);
        ...
        private void GenerateSegmentoAr()
        {
            AddGenericCompleteSegment(_h400.TIPO_DOC, Header.REFE, _h400.TIPOFACT,
                ...
                _h400.Cliente.Value, 0, "T0400_FACTURA_H", _idFactura);
        }
        ...
        private void GeneraSegmentoFromFacturaItems()
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                foreach (var item in factItems)
                {
                    ...
                    if (item.PRECIOT_FACT_ARS == null)
                    {
                        var importeX = item.PRECIOT;
                        importeDH = importeX.Value;
                        //esto es un fix! retirarlo todo retirar este fix
                    }
                    else
                    {
                        importeDH = _h400.FacturaMoneda == "ARS" ? item.PRECIOT_FACT_ARS.Value : item.PRECIOT_FACT_USD.Value;
                    }

                   

                    AddGenericCompleteSegment(_h400.TIPO_DOC, Header.REFE, _h400.TIPOFACT, item.GLV, descripcion1,
                        descripcion2, _h400.FacturaMoneda, DebeHaber.Haber, importeDH, base.Tcode, _h400.Cliente.Value,
