            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                switch (tipoLx)
                {
                    case "L1":
                        ...
                        //var data = db.T0602_DOCU_S
                        //    .Select(i => new { i.PERIODO, i.GL, i.TIPO, i.DEBE, i.HABER })
                        //    .Where(c => c.PERIODO == periodoGet && c.GL == cuentaGL && c.TIPO == tipoLx)
                        //    .GroupBy(l => l.PERIODO).Select(cl => new RetornoDh()
                        //    {
                        //        Periodo = periodoGet,
                        //        TipoLx = tipoLx,
                        //        Debe = cl.Sum(c => c.DEBE.Value),
                        //        Haber = cl.Sum(c => c.HABER.Value)
                        //    }).ToList();
                        //if (data.Any())
                        //{
                        //    rtn.Debe += data[0].Debe;
                        //    rtn.Haber += data[0].Haber;
                        //}
