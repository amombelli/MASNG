    ...
    public class GLAccountManagement
    {
        ...
        public string GetApVendorGl(int idProveedor)
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                var dataProvType = new VendorManager().GetDataVendorType(dataProv.TIPO);
...
#pragma warning restore CS0168 // The variable 'tipoPrimario' is declared but never used
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                if (materialData != null)
                {
                    ...
                    var dataTipo = db.T0012_TIPO_MATERIAL.SingleOrDefault(c => c.TIPO_MATERIAL.ToUpper().Equals(tipoAka.ToUpper()));
                    if (dataTipo == null)
                        return "0.0.0.0";
                    return dataTipo.GL;
                }
                else
                {
                    ...
                    {
                        var dataTipo = db.T0012_TIPO_MATERIAL.SingleOrDefault(c => c.TIPO_MATERIAL.ToUpper().Equals(dataPrimario.TIPO_MATERIAL.ToUpper()));
                        if (dataTipo == null)
                            return "0.0.0.0";
                        return dataTipo.GL;
                    }
