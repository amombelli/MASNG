    ...
    public class StockBatchManageRE
    {
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                var data =
                    db.T0030_STOCK.Where(
                        c =>
                            c.OV_Reserva == itemR.IDOV && c.Material.ToUpper().Equals(itemR.MATERIAL.ToUpper()) &&
                ...
                if (data.Any())
                {
                    ...
                    if (itemR.KGDESPACHADOS.Value > data[0].Stock)
                    {
                        //split lista items
                        kgPendientes = itemR.KGINI.Value - data[0].Stock;
                    ...
                    else if (itemR.KGDESPACHADOS.Value == data[0].Stock)
                    {
                    ...
                    {
                        new StockManager().TomaLineaStock(data[0].IDStock, itemR.KGINI.Value,
                            StockStatusManager.EstadoLote.ReservaRE, remitoGuid: itemR.NUMREMITO);
            ...
            ModoBusqueda modo = ModoBusqueda.ChicoGrande)
        {
            ...
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                decimal kgPendientesAsignacion = itemR.KGDESPACHADOS.Value;
                ...
                else
                {
                    if (itemR.KGDESPACHADOS.Value < data[0].Stock)
                    {
                        ...
                        new StockManager().TomaLineaStock(data[0].IDStock, itemR.KGDESPACHADOS.Value,
                            StockStatusManager.EstadoLote.ReservaRE, remitoGuid: itemR.NUMREMITO, reservadoOV: lst[0].IDOV, reservaItem: lst[0].IDOVITEM);
            ...
            ModoBusqueda searchMode = ModoBusqueda.GrandeChico)
        {
            ...
            var kgPendientesAsignacion = itemR.KGDESPACHADOS.Value;
            var lst = new List<T0056_REMITO_I> {itemR};
