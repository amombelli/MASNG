...
namespace Tecser.Business.Transactional.MM
{
    public class MmLog
    {
        ...
        public int LogMMCancelacionRemito(int idRemito, int idItem)
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                var logOut = LogMovimientoT40(x.MATERIAL, DateTime.Today, Convert.ToInt32(tipoMovimiento), "CANR",
                    x.NUMREMITO, x.KGDESPACHADOS.Value, "CANR", x.SLOC,
        ...
        public int LogMMAltaNewStockDevolucion(MotivoDevolucion.Motivo motivo, int idHeaderDev)
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                ...
                switch (motivo)
                {
                    case MotivoDevolucion.Motivo.ErrorAdmnitrativo:
                        break;
                ...
                var logOut = LogMovimientoT40(x.MATERIAL, x.FECHA.Value, Convert.ToInt32(tipoMovimiento), "RTN",
                    idHeaderDev.ToString(), x.KG.Value, "RTN1", "LAB1",
                    StockStatusManager.EstadoLote.Restringido.ToString(), "I", "LX", x.LOTE, idCli: x.IDCLI.Value,
