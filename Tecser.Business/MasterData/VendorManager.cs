...
namespace Tecser.Business.MasterData
{
    public class VendorManager
    {
        ...
        public T0014_TIPO_PROVEEDOR GetDataVendorType(string vendorType)
        {
        ...
        public List<T0014_TIPO_PROVEEDOR> GetListOfVendorType(bool soloActivo = false)
        {
            return soloActivo == true
                ? new TecserData(GlobalApp.CnnApp).T0014_TIPO_PROVEEDOR.Where(c => c.Activo == true).ToList()
                : new TecserData(GlobalApp.CnnApp).T0014_TIPO_PROVEEDOR.ToList();
        }
        ...
        public T0005_MPROVEEDORES GetSpecificVendor(int idVendor)
        {
            ...
            if (data == null)
            {
                var vendornull = new T0005_MPROVEEDORES {id_prov = -1};
                return vendornull;
            }
            return data;
        }
        ...
        public int GuardaCambiosVendor(T0005_MPROVEEDORES p)
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                if (p.id_prov == 0)
                {
                    ...
                    p.LOG_USER = Environment.UserName;
