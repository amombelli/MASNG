...
namespace Tecser.Business.MasterData
{
    public class CustomerManager
    {
        ...
        public void SetClienteActivo(int idCliente)
        {
        ...
        public void SetClienteInactivo(int idCliente, bool desactivaAllShipTos = false,bool checkSaldo=true)
        {
            using (var db = new TecserData(GlobalApp.CnnApp))
            {
                var cli = db.T0006_MCLIENTES.SingleOrDefault(c => c.IDCLIENTE == idCliente);
                cli.ACTIVO = false;
                if (desactivaAllShipTos)
                {
