...
namespace Tecser.Business.MasterData
{
    public class CuentasManager
    {
        ...
        {
            return new TecserData(GlobalApp.CnnApp).T0150_CUENTAS.Where(c => c.DisponibleCobranza == true).ToList();
        }
        public List<T0150_CUENTAS> GetListCuentasAvailableOP()
        {
            return new TecserData(GlobalApp.CnnApp).T0150_CUENTAS.Where(c => c.DisponibleOP == true).ToList();
        }
        public List<T0150_CUENTAS> GetListCuentasAvailableTransferencia()
        {
            return new TecserData(GlobalApp.CnnApp).T0150_CUENTAS.Where(c => c.DisponibleREG.Value == true).ToList();
        }

        public List<T0150_CUENTAS> GetListaCuentasAvailableForContar(string moneda="ARS")
        {
            return new TecserData(GlobalApp.CnnApp).T0150_CUENTAS.Where(c => c.DisponibleContar.Value == true && c.CUENTA_MONEDA.ToUpper().Equals(moneda.ToUpper())).ToList();
        }
