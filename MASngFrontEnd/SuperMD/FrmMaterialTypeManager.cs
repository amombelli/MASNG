...
namespace MASngFE.SuperMD
{
    public partial class FrmMaterialTypeManager : Form
    {
        ...
        private T0012_TIPO_MATERIAL MapScreenData()
        {
            var data = new T0012_TIPO_MATERIAL()
            {
        ...
        {
            var data = new MaterialTypeManager().GetMaterialTypeData(cmbMaterialType.SelectedValue.ToString());
            if (data == null)
            {
                BlanqueaDatos();
                return;
        ...
        {
            MaterialTypeBs.DataSource = new MaterialTypeManager().GetAllAvailableMaterialType();
            cmbMaterialType.SelectedIndex = -1;
