...
namespace MASngFE._UserControls
{
    public partial class UVendorSearch : UserControl
    {
        ...
        private bool _cuitValido;
        private DelEventHandler _eventoVendorSelectionChanged;  //modificado 27.08.19

        //---------------------------------------------------------------------------------------------------------------------

        public event DelEventHandler Show
        {
            add => _eventoVendorSelectionChanged += value;
            remove
            {
                if (value != null) _eventoVendorSelectionChanged -= value;
            }
        }
        ...
        private void txtTaxNumber_Leave(object sender, EventArgs e)
        {
            ...
            if (_cuitValido == true)
            {
                var vxlist = new TecserData(GlobalApp.CnnApp).T0005_MPROVEEDORES.Where(c => c.NTAX1.Equals(txtTaxNumber.Text)).ToList();
                if (vxlist.Count == 1)
                {
                    cmbRazonSocial.SelectedValue = vxlist[0].id_prov;
                }
            }
        }
        //
        ...
        private void cmbRazonSocial_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ValidaCuitCorrecto();
            _eventoVendorSelectionChanged?.Invoke(); //modificado 27.08.19
        }
        ...
        public void InicializaUc(bool ckHabilitado = true, bool ckValue = false, Color colorPanel = default(Color))
        {
            ...
            T0014VendorTypeBs.DataSource = new TecserData(GlobalApp.CnnApp).T0014_TIPO_PROVEEDOR.ToList();
