...
namespace MASngFE.Application
{
    public partial class FrmFI78ImportacionComprobantesAFIP : Form
    {
        ...
        private void ImportarConvertirDatosAFIP()
        {
            ...
            do
            {
                ...
                var row = dt.NewRow();
                {
                    ...
                    row["Status"] = ChequeaInfo(row["NTAX"].ToString(), doc);
                }
                dt.Rows.Add(row);
                i++;
        ...
        private string ChequeaInfo(string cuit, string documento)
        {
            var z = new ValidacionFaturasAfip().FacturaContabilizada(cuit, documento);
