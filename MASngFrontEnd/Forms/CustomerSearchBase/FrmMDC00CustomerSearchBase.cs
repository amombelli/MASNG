...
{
    public partial class FrmMdc00CustomerSearchBase : Form
    {
        protected FrmMdc00CustomerSearchBase()
        {
            InitializeComponent();
            //
            ...
            this.txtTaxNumber.DataBindings.Add(new System.Windows.Forms.Binding("Text", this.T0006Bs, "CUIT", true));
            //
...
#pragma warning disable CS0169 // The field 'FrmMdc00CustomerSearchBase._funcion' is never used
        private string _funcion;
#pragma warning restore CS0169 // The field 'FrmMdc00CustomerSearchBase._funcion' is never used
#pragma warning disable CS0169 // The field 'FrmMdc00CustomerSearchBase._modo' is never used
        private readonly int _modo;
#pragma warning restore CS0169 // The field 'FrmMdc00CustomerSearchBase._modo' is never used
        ...
        protected void cmbRazonSocial_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbRazonSocial.SelectedIndex == -1)
            {
                ...
                T0006DgvBs.DataSource = _customerList.Where(c => c.IDCLIENTE ==-1).ToList();
                return;

            }
            Id6 = Convert.ToInt32(cmbRazonSocial.SelectedValue);
            cmbFantasia.SelectedValue = cmbRazonSocial.SelectedValue;
            T0006DgvBs.DataSource = _customerList.Where(c => c.IDCLIENTE == Id6.Value).ToList();
        ...
        private void FrmCustomerSearchBase_Load(object sender, EventArgs e)
        {
            if (System.ComponentModel.LicenseManager.UsageMode != System.ComponentModel.LicenseUsageMode.Designtime)
            {
                //do connection stuff here
                _customerList = new CustomerManager().GetCompleteListofBillTo();
                T0006Bs.DataSource = _customerList;
                ckSoloActivos.Checked = true;
                txtNumeroClientesList.Text = @"3";
                BlanqueaSeleccion();
            }
        }
        ...
        private void cmbRazonSocial_KeyUp(object sender, KeyEventArgs e)
        {
            if (cmbRazonSocial.SelectedIndex == -1)
            {
                if (cmbRazonSocial.Text.Length >= Convert.ToInt32(txtNumeroClientesList.Text))
                {
                    ...
                    T0006DgvBs.DataSource =
                        new TecserData(GlobalApp.CnnApp).T0006_MCLIENTES.Where(
        ...
        private void txtCustomerId6_KeyUp(object sender, KeyEventArgs e)
        {
            if (string.IsNullOrEmpty(txtCustomerId6.Text))
            {
                ...
                BlanqueaSeleccion();
                T0006DgvBs.DataSource = _customerList;
