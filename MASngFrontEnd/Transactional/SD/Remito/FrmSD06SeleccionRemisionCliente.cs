    ...
    public partial class FrmSD06SeleccionRemisionCliente : Form
    {
        ...
        private void cmbRazonSocial_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbRazonSocial.SelectedIndex > 0)
            {
                ...
                //var cliSeleccionado = new CustomerManager().GetCustomerShipToData()
        ...
        private void AsignaStatusItemsRemitoListaMemoria()
        {
            ...
            foreach (var xitem in _itemList)
            {
                ...
                else
                {
                    ...
                    if (StockTotal > 0)
                    {
                        if (xitem.KGDESPACHADOS.Value <= (decimal) StockLiberado)
                        {
        ...
        private void AddLineaTotal()
        {
            ...
            {
                kgRemito += item.KGDESPACHADOS.Value;
                if (item.STATUSITEM != "UNKNOWN")
                    kgConLote += item.KGDESPACHADOS.Value;
            }
        ...
        private void btnSalir_Click(object sender, EventArgs e)
        {
            ...
            {
                btnCancelarAsignacion.Enabled = true;
                MessageBox.Show(@"La Lista contiene CostItems. Antes de salir debe cancelar la Asignacion",
                    @"Lista contiene CostItems", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            this.Close();
        }
...
#pragma warning restore CS1690 // Accessing a member on 'FrmSeleccionBatchDespacho.IdStockSeleccionado' may cause a runtime exception because it is a field of a marshal-by-reference class
        ...
        private void RemoveItemRemitoMemoria(int idItemRemito)
        {
            ...
            {
                new StockBatchManagerSD().LiberaStockComprometido(item.idStockComprometido.Value);
            }
        ...
        private void ConsolidaItemsEnMemoria()
        {
            ...
            foreach (var item in _itemList.Where(c=>c.BATCH==null))
            {
                ...
                if (lista.Count > 0)
                {
                    ...
                    {
                        kg += lista[i].KGDESPACHADOS.Value;
                        lista[i].RLINK = 999;
                    }
                ...
                if (new RemitoItemManager().SetItemPreparacion(item) == false)
                {
                    dr =MessageBox.Show(
                        string.Format(
                            ...
                            item.KGDESPACHADOS.Value.ToString("N2")), @"Item sin LOTE ASIGNADO", MessageBoxButtons.YesNo,
        ...
        {
            //var pregunta = MessageBox.Show(@"Confirma el cierre del módulo?", @"Cierre", MessageBoxButtons.YesNo,
            //    MessageBoxIcon.Question);
