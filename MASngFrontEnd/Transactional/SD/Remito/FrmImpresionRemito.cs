...
namespace MASngFE.Transactional.SD.Remito
{
    public partial class FrmImpresionRemito : Form
    {
        ...
        private void LoadRemitoData()
        {
            ...
            dtpFechaRemito.Value = dataH.FECHA ?? DateTime.Today;
            ckRemitoImpreso.Checked = dataH.Impreso != null && dataH.Impreso.Value;
            ...
            var dataCliente7 = new CustomerManager().GetCustomerShipToData(dataH.CODCLIENTREGA.Value);
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(@"El Remito ya se encuentra IMPRESO", @"Impresion de Remito", MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
                return;
            }
            if (txtNumeroRemito.Text.Contains("X") || txtNumeroRemito.Text.Contains("x"))
            {
                MessageBox.Show(@"El Numero de Remito es Incorrecto.- Remueva X verificando primero que el numero del sistema coincide con el numero pre-impreso en el Remito", @"Impresion de Remito",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                ckNumeroRemitoOK.Checked = false;
                ckNumeroRemitoOK.BackColor = Color.LightPink;
                return;
