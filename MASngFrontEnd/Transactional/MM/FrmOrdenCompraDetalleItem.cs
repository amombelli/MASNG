...
#pragma warning restore CS0649 // Field 'FrmOrdenCompraDetalleItem._ocHeader' is never assigned to, and will always have its default value null
            ...
            if (rb.Checked)
            {
                switch (rb.Name)
                {
                    case "rbSoloProveedor":
                        ...
                        break;
                    case "rbTodos":
                        cmbMaterial.DataSource = new MaterialMasterManager().GetListMaterialAvailableToBuy();
        ...
        private T0061_OCOMPRA_I MapFormToStructureT0061()
        {
            ...
            {
                IDITEMCOMPRA = txtdItem.Text == @"Sin Definir" ? 0 : _idItem, ORDENCOMPRA = _numeroOC, MATERIAL = cmbMaterial.Text, MATERIAL_PROVEEDOR = txtMaterialProveedor.Text, REQUISICION = 0, CANTIDAD = con.GetDecimal(txtKg.Text), PRECIO_UNITARIO_P = con.GetDecimal(txtPrecioARS.Text), PRECIO_UNITARIO_D = con.GetDecimal(txtPrecioUSD.Text)
            };
