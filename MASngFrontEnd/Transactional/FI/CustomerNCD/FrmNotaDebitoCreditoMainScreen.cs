...
namespace MASngFE.Transactional.FI.CustomerNCD
{
    ...
    public partial class FrmNotaDebitoCreditoMainScreen : Form
    {
        public FrmNotaDebitoCreditoMainScreen(int idCliente, string tipoLx, ManageDocumentType.TipoDocumento tipoDocumento,
            string indicador)
        {
        ...
        public FrmNotaDebitoCreditoMainScreen(int idNcd)
        {
        ...
        private void LoadInitialDataFromExistingDocument()
        {
            ...
            txtTipoCambio.Text = t400Data.TC.Value.ToString("N2");
            ...
            _idCliente = t400Data.Cliente.Value;
            ...
            _tipoDocumento = ManageDocumentType.GetTipoDocumentoFromTDoc(ncdData.TDOC, _tipoLx);
        ...
        private void LoadHeaderDataFromDocumento()
        {
            ...
            _tipoDocumento = ManageDocumentType.GetTipoDocumentoFromTDoc(t400Data.TIPO_DOC, _tipoLx);
            ...
            var ncdH = new XContabilizaNotaCredito(_facturaIdStruct.IdFactura).GetDocumentoHeader();
         //   txtNumeroDocumento.Text = ncdH.NDOC;
           ...
           // txtNumeroDocumento.Text = @"NO-ASIGNADO";
            //txtTipoDocumento.Text = _tipoDocumento.ToString();
            dtpFechaDocumento.Value = t400Data.FECHA.Value;
            txtTipoCambio.Text = t400Data.TC.Value.ToString("C2");
            txtDireccionFiscal.Text = cli.Direccion_facturacion;
            txtLocalidad.Text = cli.Direfactu_Localidad;
            txtProvincia.Text = cli.Direfactu_Provincia;
            txtCP.Text = cli.ZIP;
        ...
        private void btnSetContabilizada_Click(object sender, EventArgs e)
        {
            ...
            {
                var conta = new XContabilizaNotaCredito(_facturaIdStruct.IdFactura).ContabilizacionCompleta();
                if (conta.IdCtaCte > 0)
                {
                    txtIdCtaCte.Text = conta.IdCtaCte.ToString();
                    txtNAS.Text = conta.NumeroAsientoIdDocu.ToString();
        ...
        private void AccionNcMaterial_Kg()
        {
            ...
            using (var f0 = new FrmSeleccionMaterialNcd(_idCliente, _tipoLx))
            {
        ...
        {
            using (var f0 = new FrmSeleccionAnulaFactura(_idCliente, _tipoLx))
            {
                DialogResult dr = f0.ShowDialog();
                if (dr == DialogResult.OK)
                {
                    ...
                    AddRegistroAnulaFactura((int)_idFacturaSeleccionada);
                    AsignaNumeroItem();
                    _docData.CreateNewItemListInMemory(_listaItems); //:)
                    var r = _docData.UpdateHeaderTotalesWhenAnulaFacturaCompleta(_listaItems[0].IDCHE.Value);
                    if (r == false)
                    {
        ...
        private void AccionNcDiferenciaPrecio()
        {
            ...
            using (var f0 = new FrmSeleccionDocAjustePrecioU(_tipoDocumento, _tipoLx, _idCliente))
            {
                DialogResult dr = f0.ShowDialog();
                if (dr == DialogResult.OK)
                {
        ...
        private void AccionNcAjusteTC()
        {
          ...
          using (var f0 = new FrmSeleccionDocumentoAjusteTipoCambio(_tipoDocumento, _tipoLx, _idCliente))
            {
                DialogResult dr = f0.ShowDialog();
                if (dr == DialogResult.OK)
                {
        ...
        private void AddRecordDiferenciaPrecioUnitario(int idFactura, int idItem,decimal nuevoPrecioU)
        {
            ...
            var headerFacturaOri = new NCDAnulaDocumento().GetDatosFactura(idFactura);
        ...
        {
            var dataH = new NCDAnulaDocumento().GetDatosFactura(idFactura);
        ...
        private void AddRegistroAnulaFactura(int idFactura)
        {
            var dataF = new NCDAnulaDocumento().GetDatosFactura(idFactura);
            var NumeroDocumento = new CustomerInvoice("NCD", idFactura).GetNumeroDocumentoCompleto();
            var descripcion = $"Anulacion Documento: {dataF.TIPO_DOC} # {NumeroDocumento}";

            var dataI = new NCDAnulaDocumento().GetItemsDocumentoSeleccionado(idFactura).ToList();
        ...
        {
            if (!PermiteSolicitarCAE())
            {
                MessageBox.Show(@"El Documento no se encuentra en condiciones de solicitar CAE",
                    @"Error en solicitud de CAE", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            var dr =
               MessageBox.Show(
                   string.Format(
                       ...
                       txtTotalFactura.Text), @"SOLICITUD DE CAE", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            else
            {
                ...
                var rpt = MessageBox.Show(
                    @"Atencion se esta ejecutando la aplicacion en modo TESTEO. El CAE otorgado sera invalido. Desea Continuar?",
            ...
            var resultado = fe.SolicitudCAEFromT0400(_facturaIdStruct.IdFactura);

            if (resultado.Resultado == "A")
            {
                txtCAE.Text = resultado.CAE;
                txtCAEVencimiento.Text = resultado.VencimientoCAE.ToString("d");
                txtNumeroDocumento.Text = resultado.PuntoVenta.PadLeft(4, '0') + @"-" + resultado.ComprobanteHasta.PadLeft(8, '0');
                fe.UpdateDataAfterDocumentNumberGetFromAFIP(_facturaIdStruct.IdFactura,
                    resultado.PuntoVenta.PadLeft(4, '0'), resultado.ComprobanteHasta.PadLeft(8, '0'), resultado.CAE,
                ...
                if (_idChequeRechazado > 0)
                    new ChequeRechazadoManager().CompleteNumeroDocumentoCompletoAfterCAE(_idChequeRechazado.Value, txtNumeroDocumento.Text);
        ...
        private void lnkSolicitudCAE_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            SolicitudCAE();
        }
