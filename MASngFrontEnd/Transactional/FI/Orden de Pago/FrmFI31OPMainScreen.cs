...
namespace MASngFE.Transactional.FI.Orden_de_Pago
{
    public partial class FrmFI31OPMainScreen : Form
    {
        ...
        {
            var lista = _op.GetDatosFacturasPagandoFromDb().ToList();
        ...
        private bool OPCanBeInitialized()
        {
            ...
            if (DateTime.Today.AddDays(-15) > dtpFechaOP.Value)
            {
                var x = MessageBox.Show(@"La fecha de la OP es anterior a 15 dias lo que podria ser un error. Desea continuar de todas formas?",@"Error en Comprobacion de Datos", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void dgvFacturasPendientes_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (senderGrid.Columns[e.ColumnIndex] is DataGridViewButtonColumn && e.RowIndex >= 0)
            {
                ...
                var valor = dgvFacturasPendientes[6, e.RowIndex];
                    ...
                    case "AGREGAR":
                    {
                        ...
                        switch (tipoDoc)
                        {
                            ...
                            default:
                                resp =
                                    MessageBox.Show(
                                        $"Confirma agregar FACTURA {numfact.Value} por importe ${valor.Value}",
                                        @"Confirmacion", MessageBoxButtons.YesNo);
                                break;
                        }
