...
namespace MASngFE.Transactional.FI.Vendor.ConRemito
{
    public partial class FrmFI21AddItemToFactura : Form
    {
        ...
        private void MapCostUCData()
        {
            ...
            {
                var costo = db.T0033_COSTO.SingleOrDefault(c => c.MATERIAL == txtMaterialTS.Text);
                if (costo == null)
                {
                    txtUCFecha.Text = null;
                    txtUCARS.Text = 0.ToString("C2");
                    txtUCUSD.Text = 0.ToString("C2");
                    txtUCProveedor.Text = "SIN INFO";
                }
                else
                {
                    if (costo.FECHA_UC !=null)
                        txtUCFecha.Text = costo.FECHA_UC.Value.ToString("d");

                    if (costo.COSTO_UC_ARS!=null)
                        txtUCARS.Text = costo.COSTO_UC_ARS.Value.ToString("C2");

                    if (costo.COSTO_UC_USD!=null)
                        txtUCUSD.Text = costo.COSTO_UC_ARS.Value.ToString("C2");

                    if (costo.Proveedor !=null)
                        txtUCProveedor.Text =new VendorManager().GetSpecificVendor(costo.Proveedor.Value).prov_rsocial.ToString();
                }
            }
        }
