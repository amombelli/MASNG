...
{
    public partial class FrmCustomerDocumentSearch : Form
    {
        public FrmCustomerDocumentSearch()
        {
            InitializeComponent();
        ...
        private void cmbRazonSocial_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            dgvDocumentHeader.DataSource = new CustomerDocumentSearch().GetListDocument(_id6);
        }
        ...
        private void FrmCustomerDocumentSearch_Load(object sender, EventArgs e)
        {
        ...
        private void dgvDocumentHeader_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            {
                ...
                if (senderGrid.Columns[e.ColumnIndex] is DataGridViewButtonColumn &&
                    e.RowIndex >= 0)
                {
                    var cellValue = dgvDocumentHeader[e.ColumnIndex, e.RowIndex].Value.ToString();
                    int idFactura =
                        Convert.ToInt32(
                            dgvDocumentHeader[
                                dgvDocumentHeader.Columns[iDFACTURADataGridViewTextBoxColumn.Name].Index, e.RowIndex]
                                .Value);

                    switch (cellValue)
                    {
                        case "VER":
                            using (var f0 = new FrmDetalleDocumentoFI(idFactura))
                            {
