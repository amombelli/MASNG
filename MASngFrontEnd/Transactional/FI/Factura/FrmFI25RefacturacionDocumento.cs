...
namespace MASngFE.Transactional.FI.Factura
{
    public partial class FrmFI25RefacturacionDocumento : Form
    {
        ...
        private void MapDatosDocumentOriginal()
        {
            ...
            txtIdCliente.Text = _fH.Cliente.Value.ToString();
            txtFechaDocOri.Text = _fH.FECHA.Value.ToString("d");
            ...
            txtTCOri.Text = _fH.TC.Value.ToString("N2");
        ...
        {
            decimal vInicial = _fI.Sum(c => c.PRECIOT_FACT_ARS.Value);
            ...
            foreach (var items in _fI)
            {
                ...
                {
                    vBaseImponible += items.PRECIOT_FACT_ARS.Value;
                }
        ...
        private void btnSetEstadoRegistrado_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in _fI)
            {
                _docData.AddItemListToMemorySimple(item.ITEM, item.DESC_REMITO, item.KGDESPACHADOS_R.Value, item.IVA21.Value, item.MONEDA_COTIZ, item.PRECIOU_COTIZ, item.PRECIOU_FACT_ARS.Value, item.GLI, item.GLV);
            }
        ...
        private void btnSolicitarCAE_Click(object sender, EventArgs e)
        {
            ...
            if (fe.PermiteSolicitarCAE(_facturaIdStruct.IdFactura))
            {
                var dr =
                    MessageBox.Show(
