...
namespace MASngFE.Transactional.FI.Cobranza
{
    public partial class FrmDetalleCobranzaIngresada : Form
    {
        ...
        private void LoadHeaderData()
        {
            ...
            txtLogDate.Text = cobH.LogDate.Value.ToString("g");
            txtLogUser.Text = cobH.LogUser.ToString();
            ckCancel.Checked = cobH.DOC_CANCELADO.Value;
            if (ckCancel.Checked)
        ...
        {
            int i = 0;
            while (!new ImputacionCobranzas().ImputaCobranzaAutomatica(_idCobranza, ModoImputacion.Completa) == false)
            {
                i++;
            }
        ...
        {
            var f0 = new FrmDetalleImputacionPorRecibo(_idCobranza);
            f0.ShowDialog();
        }
