    ...
    public partial class FrmImputacionCobranzas : Form
    {
        ...
        private void GetCobranzaData()
        {
            dgvCobranzas.DataSource =
                new ImputacionCobranzas().GetCobranzasPendientesImputacion(_clienteSeleccionado).OrderBy(c => c.FECHA).ToList();
            dgvDocumentosAImputar.DataSource =
                new ImputacionCobranzas().GetDocumentosAImputar(_clienteSeleccionado).OrderBy(c => c.FECHA_FACT).ToList();
        }
        ...
        private void LoadCustomerData()
        {
            ...
            txtSinImputar.Text = new ImputacionCobranzas().GetSaldoAImputar208(_clienteSeleccionado).ToString("C2");
        }
        ...
        private void dgvCobranzas_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
           BlanqueaDatosCobranzasAImputar();
            ...
            var impMax =
                Convert.ToDecimal(
                    dgvCobranzas[dgvCobranzas.Columns["mONTODataGridViewTextBoxColumn"].Index, e.RowIndex].Value);
            ...
            dgvDocumentosAImputar.DataSource = new ImputacionCobranzas().GetDocumentosAImputar(_clienteSeleccionado,
        ...
        private void dgvDocumentosAImputar_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var importePendienteImpu = Convert.ToDecimal(dgvDocumentosAImputar[dgvDocumentosAImputar.Columns["mONTODataGridViewTextBoxColumn1"].Index, e.RowIndex].Value);
            txtImportePendienteImpu.Text =importePendienteImpu.ToString("C2");
            txtImportePercepcionImpu.Text = 0.ToString("C2");

            var importeOriginal = Convert.ToDecimal(dgvDocumentosAImputar[dgvDocumentosAImputar.Columns["fACTMONTODataGridViewTextBoxColumn"].Index, e.RowIndex].Value);
        ...
        {
            new ImputacionCobranzas().ImputarCobranza(Convert.ToInt32(txtIdCobranzaSeleccionada.Text),
                FormatAndConversions.CCurrencyADecimal(txtImporteAImputar.Text), Convert.ToInt32(txtIdCtaCteImpu.Text),
                Convert.ToInt32(txtSplitImpu.Text),FormatAndConversions.CCurrencyADecimal(txtPercPendientePago.Text));
            ...
            else
            {
                ...
                {
                    MessageBox.Show(
                        @"No se pueden calcular los dias de cancelacion del documento porque la imputacion está incompleta",
                        @"Datos no calculados", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show(
            ...
            //    $"Dias Imputacion [Factura A Recibo]= {z.DiasPP_FacturaRecibo} -- Dias Promedio Cobranza [Ponderado Recibo]= {z.DiasPP_ReciboPago}");
        ...
        private void btnImputacionAutomatica_Click(object sender, EventArgs e)
        {
            ...
            int i=0;
            while (!new ImputacionCobranzas().ImputaCobranzaAutomatica(idCob,ModoImputacion.Completa) == false)
            {
                i++;
            }
