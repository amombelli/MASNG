...
#pragma warning restore CS0414 // The field 'FrmIngresoCobranza._dataValidatedAfterDgv' is assigned but its value is never used
        ...
        public void CompletaFormateaDataGridView()
        {
            ...
            foreach (DataGridViewRow row in dgvCobranza.Rows)
            {
                ...
                {
                    row.Cells[2].Value =
                        new CuentasManager().GetSpecificCuentaInfo(row.Cells[1].Value.ToString()).CUENTA_DESC;
                }

                if (row.Cells[1].Value.ToString() == "CHE")
                {
                    row.Cells[0].Style.BackColor = Color.Gray; //linea
                    row.Cells[1].Style.BackColor = Color.DarkCyan; //cuenta
                    row.Cells[2].Style.BackColor = Color.LightSlateGray; //descripcion
                    row.Cells[3].Style.BackColor = Color.LightSlateGray; //mon
                    row.Cells[4].Style.BackColor = Color.White; //importe
                    row.Cells[5].Style.BackColor = Color.White; //num cheque
                    row.Cells[6].Style.BackColor = Color.White; //fecha acred
                    row.Cells[7].Style.BackColor = Color.White; //cuit
                    row.Cells[8].Style.BackColor = Color.LightSlateGray; //titular
                    row.Cells[9].Style.BackColor = Color.White; //ch interior
                    row.Cells[10].Style.BackColor = Color.White; //ch bank
                    row.Cells[11].Style.BackColor = Color.LightSlateGray; //ch bank descripcion
                    row.Cells[12].Style.BackColor = Color.White; //ch idch
                    row.Cells[13].Style.BackColor = Color.Chartreuse; //ch importe recibo
                    ...
                    if (row.Cells[10].Value != null && row.Cells[11].Value == null)
                    {
                        row.Cells[11].Value =
                            new BankManager().GetBankData(row.Cells[10].Value.ToString()).BCO_SHORTDESC;
                    }
                }
                else
                {
                    //Otras cuentas excepto cheques
                    row.Cells[0].Style.BackColor = Color.Gray; //linea
                    row.Cells[1].Style.BackColor = Color.DarkCyan; //cuenta
                    row.Cells[2].Style.BackColor = Color.LightSlateGray; //descripcion
                    row.Cells[3].Style.BackColor = Color.LightSlateGray; //mon
                    row.Cells[4].Style.BackColor = Color.White; //importe
                    row.Cells[5].Style.BackColor = Color.DimGray; //num cheque
                    row.Cells[6].Style.BackColor = Color.DimGray; //fecha acred
                    row.Cells[7].Style.BackColor = Color.DimGray; //cuit
                    row.Cells[8].Style.BackColor = Color.DimGray; //titular
                    row.Cells[9].Style.BackColor = Color.DimGray; //ch interior
                    row.Cells[10].Style.BackColor = Color.DimGray; //ch bank
                    row.Cells[11].Style.BackColor = Color.DimGray; //ch bank descripcion
                    row.Cells[12].Style.BackColor = Color.DimGray; //ch idch
                    row.Cells[13].Style.BackColor = Color.Chartreuse; //ch importe recibo
                }
            }
        }
        ...
        private void dgvCobranza_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvCobranza.CurrentCell.ColumnIndex == 4)
            {
            ...
            if (dgvCobranza.Rows[e.RowIndex].Cells[1].Value.ToString() == "CHE")
            {
                if (dgvCobranza.CurrentCell.ColumnIndex == 7) //CUIT
                {
                ...
                if (dgvCobranza.CurrentCell.ColumnIndex == 10) //Banco
                {
                ...
                if (dgvCobranza.CurrentCell.ColumnIndex == 6) //Fecha Acreditacion
                {
                }
            }
            else
        ...
        public void UpdateListWithCalculosImportes()
        {
            ...
            foreach (var item in ListOfItemsCobranza)
            {
                ...
                totalCob = totalCob + item.IMP_RECIBO.Value;
            }
        ...
        private void btnIngresar_Click(object sender, EventArgs e)
        {
            if (ValidaDatosHeaderCobranza())
            {
                ...
                if (pl == null)
                {
                    ...
                    GescoPagoListo.SetFechaUltimaCobranza(_idCliente.Value,dtpFechaCobranza.Value);
                }
                else
                {
        ...
        private void IngresaCobranza()
        {
            ...
            foreach (var i in ListOfItemsCobranza)
            {
                ...
                if (i.CUENTA == "CHE")
                {
                    //todo convertir campo interior a bool en tabla items cobranza!

                    bool chequeEsInterior = false;
                    if (i.CHEQUE_INTERIOR == "SI")
                    {
                        chequeEsInterior = true;
                    }

                    var idChequeAdded = new ChequesManager().AddNewChequeConstructor(_tipoLx,
                        txtNumeroReciboInterno.Text,
                        dtpFechaCobranza.Value, Convert.ToInt32(txtIdCliente.Text), i.CHEQUE_FECHA.Value,
                        i.IMP_RECIBO.Value, i.CHEQUE_NUMERO, i.CHEQUE_BANCO, chequeEsInterior, i.CHEQUE_CUIT,
                        i.CHEQUE_TITULAR, i.MON_ITEM, i.COMENTARIO, _idCob, Convert.ToInt32(txtIdCliente.Text), null);

                    documento.AddCobranzaItemCheque(idChequeAdded, i.IMP_RECIBO.Value, i.TC_ITEM.Value, i.COMENTARIO);
                }
                else
                {
                    documento.AddCobranzaItem(i.CUENTA, i.MON_ITEM, i.IMP_ITEM.Value, i.IMP_RECIBO.Value,
            ...
            var asientoCobranza = new AsientoCobranza(resultado.IdCobranza, "COBI1");
        ...
        private List<T0206_COBRANZA_I> MapItemsCobranzaFromTemporal()
        {
            ...
            foreach (var i in idata)
            {
                ...
                if (i.ChequeInterior)
                {
                    l0.CHEQUE_INTERIOR = "SI";
                }
                else
                {
                    l0.CHEQUE_INTERIOR = "NO";
                }
        ...
        private void dgvCobranza_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (senderGrid.Columns[e.ColumnIndex] is DataGridViewButtonColumn && e.RowIndex >= 0)
            {
                ...
                switch (cellValue)
                {
                    case "DEL":
                    {
                        var iditem = Convert.ToInt32(dgvCobranza[lINEDataGridViewTextBoxColumn.Name, e.RowIndex].Value);
        ...
        private void dgvCobranza_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            if (dgvCobranza.CurrentCell.ColumnIndex == 4)
            {
                ...
                if (string.IsNullOrEmpty(e.FormattedValue.ToString()))
                {
                    ...
                    MessageBox.Show(@"El Importe NO PUEDE ser Nulo", @"Error en Importe", MessageBoxButtons.OK,
                        MessageBoxIcon.Information);
                    ListOfItemsCobranza[e.RowIndex].IMP_ITEM = 0;
                    ListOfItemsCobranza[e.RowIndex].IMP_RECIBO = 0;
                }
                else
                {
                    ...
                    if (ListOfItemsCobranza[e.RowIndex].MON_ITEM == "USD")
                    {
                        ListOfItemsCobranza[e.RowIndex].IMP_RECIBO = valueDecimal *
                                                                     ListOfItemsCobranza[e.RowIndex].TC_ITEM.Value;
                    }
                    else
                    {
                        ListOfItemsCobranza[e.RowIndex].IMP_RECIBO = valueDecimal;
            ...
            if (dgvCobranza.CurrentCell.ColumnIndex == 6)
            {
                //Validacion de la Fecha del Cheque
                var value = e.FormattedValue.ToString();
                if (string.IsNullOrEmpty(e.FormattedValue.ToString()))
                {
                    //dgvCobranzaItems.Rows[currentcell.Y].Cells[currentcell.X].Value= null;
                    ListOfItemsCobranza[e.RowIndex].CHEQUE_FECHA = null;
                    return;
#pragma warning disable CS0162 // Unreachable code detected
                    e.Cancel = true;
#pragma warning restore CS0162 // Unreachable code detected
                    MessageBox.Show(@"La fecha no puede ser NULA", @"Error en Fecha", MessageBoxButtons.OK,
                        MessageBoxIcon.Information);
                }
                else
                {
