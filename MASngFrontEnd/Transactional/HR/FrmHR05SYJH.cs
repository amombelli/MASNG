...
namespace MASngFE.Transactional.HR
{
    public partial class FrmHR05SYJH : Form
    {
        ...
        private void btnAddMensual_Click(object sender, EventArgs e)
        {

        }

        private void btnAddQuincena_Click(object sender, EventArgs e)
        {
            this.dgvData.RowsRemoved -= new System.Windows.Forms.DataGridViewRowsRemovedEventHandler(this.dgvData_RowsRemoved);
            if (cmbConceptos.SelectedValue == null)
            {
                MessageBox.Show(@"Debe Seleccionar un CONCEPTO para agregar", @"Datos Incompletos", MessageBoxButtons.OK,
                    MessageBoxIcon.Exclamation);
                return;
            }
            var listaEmpl = new HumanResourcesManager().GetListaEmpleadosQuincena();
            foreach (var empl in listaEmpl)
            {
                var data = new T0520_SYJItems()
                {
                    idItem = _listaItems.Count + 1,
                    Cantidad = 1,
                    Aprobado = false,
                    ...
                    Descuento = 0,
                    EmpleadoShort = empl.SHORTNAME,
                    ValorUnitario = 100,
                    idH = 1,
                };
                if (empl.POSICION != null)
                    data.PosicionHR = HumanResourcesManager.GetPosicioneData(empl.POSICION.Value).PosicionDescripcion;

                data.Total = (data.Cantidad * data.ValorUnitario) - data.Descuento;

                _listaItems.Add(data);
            }
            t0520SYJItemsBindingSource.DataSource = _listaItems.ToList();
            this.dgvData.RowsRemoved += new System.Windows.Forms.DataGridViewRowsRemovedEventHandler(this.dgvData_RowsRemoved);
